...
- **rules/analyzers** apply the heuristics shipped in `embedaudit/core.py`.
- **score** ranks by severity.
- **MCP server** (`embedaudit mcp`) exposes `scan` for Cognis.Studio agents.
