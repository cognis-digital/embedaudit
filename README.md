...
[![PyPI](https://img.shields.io/pypi/v/cognis-embedaudit.svg)](https://pypi.org/project/cognis-embedaudit/)
[![CI](https://github.com/cognis-digital/embedaudit/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/embedaudit/actions)
...
EMBEDAUDIT exists for one job — embedding / vector-store drift and poisoning audit — and does it without a SaaS bill or heavyweight setup.
...
```bash
embedaudit --version
embedaudit scan .                      # scan the current project
embedaudit scan . --format json
embedaudit scan . --fail-on high       # non-zero exit for CI gates
embedaudit mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **RAG security**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`embedaudit` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
